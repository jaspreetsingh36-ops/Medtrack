...
def setup():
    try:
        ...
        # Add appointments (John Doe sees multiple doctors - JOINT PATIENT)
        cur.execute("""
            INSERT INTO appointments (patient_id, doctor_id, appointment_date, status, reason) VALUES 
            (1, 1, '2026-04-01 10:00:00', 'scheduled', 'Chest pain - Dr. Smith'),
            (1, 3, '2026-04-05 14:00:00', 'scheduled', 'Child vaccination - Dr. Brown'),
            (2, 2, '2026-04-02 11:00:00', 'scheduled', 'Migraine - Dr. Johnson'),
            (3, 1, '2026-04-03 09:00:00', 'completed', 'Fever - Dr. Smith')
        """)
        conn.commit()
        ...
        <!DOCTYPE html>
        <html>
        <head><title>Setup Complete - MedTrack</title>
        <style>
            body { font-family: Arial; text-align: center; padding: 50px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
            .container { background: white; border-radius: 10px; padding: 40px; max-width: 600px; margin: 0 auto; }
            h1 { color: #28a745; }
        ...
        <body>
            <div class="container">
                ...
                <ul style="text-align: left;">
                    ...
                    <li><strong>Dr. Smith:</strong> dr_smith / doctor123</li>
                    <li><strong>Dr. Johnson:</strong> dr_johnson / doctor123</li>
                    <li><strong>Dr. Brown:</strong> dr_brown / doctor123</li>
                </ul>
                <p><strong>Joint Patient Example:</strong> John Doe sees both Dr. Smith AND Dr. Brown</p>
...
def login():
    ...
    <body>
        <div class="container">
            ...
            <p style="text-align:center; margin-top:20px"><small>admin / admin123 | dr_smith / doctor123 | dr_johnson / doctor123</small></p>
        </div>
    </body>
    </html>
    '''
...
def dashboard():
    ...
    if session['role'] in ['admin', 'staff']:
        ...
        cur.execute("""
            ...
            WHERE d.user_id = %s
        """, (session['user_id'],))
        patient_count = cur.fetchone()['count'] or 0
        doctor_count = 0  # Doctors don't need to see other doctors count
        cur.execute("""
            SELECT COUNT(*) as count FROM appointments a
            JOIN doctors d ON a.doctor_id = d.doctor_id
            WHERE d.user_id = %s AND a.status = 'scheduled'
        """, (session['user_id'],))
        appointment_count = cur.fetchone()['count'] or 0
    
    # Get recent appointments with JOIN based on role
    if session['role'] in ['admin', 'staff']:
        cur.execute("""
    ...
    <body>
        <div class="container">
            <div class="header">
                <div>
                    <h1>🏥 MedTrack</h1>
                    <p>Welcome, {session['username']}! <strong>Role: {session['role'].upper()}</strong></p>
                </div>
                <div>
            ...
            <div class="info-box">
                ...
                { "Admin & Staff: You can see ALL patients and ALL doctors" if session['role'] in ['admin', 'staff'] else "Doctor: You can ONLY see patients you have appointments with" }
            </div>
            
            <div class="stats">
                <div class="stat-card">
                    <div class="stat-number">{patient_count}</div>
                    <div>{"Total Patients" if session['role'] in ['admin', 'staff'] else "Your Patients"}</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{doctor_count if session['role'] in ['admin', 'staff'] else "—"}</div>
                    <div>{"Total Doctors" if session['role'] in ['admin', 'staff'] else "N/A"}</div>
                </div>
                <div class="stat-card">
            ...
            <div class="card">
                ...
                <table>
                    ...
                    <tbody>
                        {''.join([f'<tr><td>{a["appointment_date"]}</td><td>{a["patient_name"]}</td><td>{a["doctor_name"]}</td><td><span class="badge badge-{a["status"]}">{a["status"]}</span></td><td>{a["reason"] or "N/A"}</td></tr>' for a in appointments])}
                    </tbody>
                </table>
            </div>
...
def list_patients():
    ...
    <head>
        ...
        <style>
            ...
            .info-box {{ background: #fff3cd; padding: 10px; border-radius: 5px; margin-bottom: 20px; border-left: 4px solid #ffc107; }}
        </style>
    </head>
    <body>
        <div class="container">
            <div class="header">
                ...
                <div>
                    ...
                    {"<a href='/patients/add' class='btn'>+ Add Patient</a>" if session['role'] in ['admin', 'staff'] else ""}
                    <a href="/logout" class="btn btn-danger">Logout</a>
                </div>
            </div>
            
            {"<div class='info-box'>ℹ️ As a doctor, you can ONLY see patients you have appointments with.</div>" if session['role'] == 'doctor' else ""}
            
            <div class="card">
                <h3>{"All Patients" if session['role'] in ['admin', 'staff'] else "Your Patients"}</h3>
                <table>
                    <thead>
                        <tr><th>ID</th><th>Name</th><th>DOB</th><th>Contact</th><th>Insurance</th>{"<th>Actions</th>" if session['role'] in ['admin', 'staff'] else ""}</tr>
                    </thead>
                    <tbody>
                        {''.join([f'<tr><td>{p["patient_id"]}</td><td>{p["name"]}</td><td>{p["dob"] or "N/A"}</td><td>{p["contact"] or "N/A"}</td><td>{p["insurance_no"] or "N/A"}</td>{"<td><a href=\"/patients/edit/" + str(p[\"patient_id\"]) + "\" class=\"btn btn-sm\">Edit</a> <a href=\"/patients/delete/" + str(p[\"patient_id\"]) + "\" class=\"btn btn-sm btn-danger\" onclick=\"return confirm(\'Delete?\');\">Delete</a></td>" if session['role'] in ['admin', 'staff'] else ""}</tr>' for p in patients])}
                    </tbody>
                </table>
            </div>
        </div>
...
def list_doctors():
    ...
    # Role-based doctor listing
    if session['role'] in ['admin', 'staff']:
        cur.execute("SELECT * FROM doctors ORDER BY name")
        doctors = cur.fetchall()
    else:  # Doctor - can only see themselves
        cur.execute("SELECT * FROM doctors WHERE user_id = %s", (session['user_id'],))
        doctors = cur.fetchall()
    ...
    <body>
        <div class="container">
            ...
            {"<div class='info-box'>ℹ️ As a doctor, you can only see your own profile.</div>" if session['role'] == 'doctor' else ""}
            
            <div class="card">
                <h3>{"All Doctors" if session['role'] in ['admin', 'staff'] else "Your Profile"}</h3>
                <table>
                    <thead>
                        <tr><th>ID</th><th>Name</th><th>Specialization</th><th>Contact</th><th>Email</th></tr>
                    </thead>
                    <tbody>
...
def list_appointments():
    ...
    if session['role'] in ['admin', 'staff']:
        cur.execute("""
            ...
            ORDER BY a.appointment_date DESC
        """)
    else:  # Doctor - only their appointments
        cur.execute("""
            SELECT a.*, p.name as patient_name, d.name as doctor_name
            FROM appointments a
            JOIN patients p ON a.patient_id = p.patient_id
...
def add_appointment():
    if request.method == 'POST':
        ...
        # For doctors, automatically use their own doctor_id
        ...
        cur.execute(
            ...
            (patient_id, doctor_id, request.form['appointment_date'], request.form.get('reason', ''))
        )
        conn.commit()
        cur.close()
        conn.close()
    ...
    if session['role'] in ['admin', 'staff']:
        ...
        doctors = cur.fetchall()
    else:  # Doctor - only patients they have appointments with
        cur.execute("""
        ...
        """, (session['user_id'],))
        patients = cur.fetchall()
        doctors = []  # Doctors can't choose another doctor
    
    cur.close()
    conn.close()
    
    patients_options = ''.join([f'<option value="{p["patient_id"]}">{p["name"]}</option>' for p in patients])
    doctors_options = ''.join([f'<option value="{d["doctor_id"]}">{d["name"]}</option>' for d in doctors]) if doctors else ''
    ...
    <body>
        <div class="container">
            ...
            <form method="POST">
                ...
                {"<select name='doctor_id' required><option value=''>Select Doctor</option>" + doctors_options + "</select>" if session['role'] in ['admin', 'staff'] else ""}
...
def add_visit():
    if request.method == 'POST':
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (patient_id, doctor_id, request.form['visit_date'],
